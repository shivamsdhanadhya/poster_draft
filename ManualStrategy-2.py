...
def testPolicy(symbol,sd,ed,sv):
	...
	lookback=20
	sym='JPM'
	sma = calc_sma(prices,lookback,symbol)
	bollinger = calc_bb(prices,symbol,lookback,sma)
	psma = calc_price_to_sma(prices, lookback, sma, symbol)
	volatility = calc_vola(prices,lookback,symbol)
	cci = calc_cci(prices,lookback,symbol)
	...
	for i in range(0, prices.shape[0]- 1):
		if flag == 0:
			if bollinger.ix[i,sym] < 0.2 or psma.ix[i,sym] < 0.6 or volatility.ix[i,sym] < -0.1 or cci.ix[i, sym] < 0:
				trades.loc[index] = [prices.index[i].strftime('%Y-%m-%d'),'JPM','BUY',1000]
				flag = 1
				index +=1
				buydate.append(prices.index[i].date())
			elif bollinger.ix[i,sym] > 0.8 or psma.ix[i,sym] > 1.1 or volatility.ix[i,sym] > 0.1 or cci.ix[i, sym] > 0:
...
if __name__ == "__main__":
	...
	print "Average of daily returns " + str(adr)
